...
from lib.rl.sac_cond import SAC_COND
from lib.rl.replay_memory import ReplayMemoryMultitask
...
def train(args):
    ...
    # Agent
    agent = SAC_COND(env.observation_space.shape[0], env.action_space, args)
    ...
    # Memory
    memory = ReplayMemoryMultitask(args.replay_size, args.seed)
    ...
    for i_episode in itertools.count(1):
        ...
        task = torch.tensor(task,dtype=int,device=device)
        ...
        while not done:
            if args.start_steps > total_numsteps:
                ...
                agent.policy.eval()
                action = agent.select_action(state, task)  # Sample action from policy
                agent.policy.train()
            ...
            memory.push(state, action, reward, next_state, info["task"], mask) # Append transition to memory
        ...
        if i_episode % 10 == 0 and args.eval is True:
            ...
            for _  in range(episodes):
                ...
                while not done:
                    action = agent.select_action(state, task, evaluate=True)
