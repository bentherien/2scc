...
from lib.rl.sac_hlp import SAC_HLP
from lib.rl.replay_memory import ReplayMemory
from lib.rl.model import modifyGaussianPolicy, modifyDeterministicPolicy
...
def train(args):
    if args.wandb:
        wandb.init(project="my-test-project", entity="bentherien")
    ...
    if args.policy == "Gaussian":
        agent.policy = modifyGaussianPolicy(agent.policy)
    else:
        print(args.policy)
        agent.policy = modifyDeterministicPolicy(agent.policy)


    higherLevelPolicy = SAC_HLP(env.observation_space.shape[0], args.hidden_size **2, args, agent.policy)
    ...
    for i_episode in itertools.count(1):
        ...
        while not done:
            if args.start_steps > total_numsteps:
                ...
                action = higherLevelPolicy.select_action(state)  # Sample action from policy

            if len(memory) > args.batch_size:
                # Number of updates per step in environment
                for i in range(args.updates_per_step):
                    # Update parameters of all the networks
                    critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = higherLevelPolicy.update_parameters(memory, args.batch_size, updates)
        ...
        if i_episode % 10 == 0 and args.eval is True:
            ...
            for _  in range(episodes):
                ...
                while not done:
                    action = higherLevelPolicy.select_action(state, evaluate=True)
        ...
        if i_episode % 250 == 0 and args.save_checkpoint is True:
            suffix = "--HLP--{}total_numsteps:{}_lastreward:{:.2f}.pt".format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), total_numsteps, avg_reward)
            higherLevelPolicy.save_checkpoint(env_name=args.env_name, suffix=suffix, testRewardData=avgRewardList)#, ckpt_path="/home/therien/Documents/github/pytorch-soft-actor-critic")
